 ...
 *
 */
public class BankSimulatorV4 {

	private static boolean isATellerAtDriveUp = false;
	...
	public static void runBank() {
		...
		try {
			...
			while (fileScanner.hasNext()) {// while there are still customers to arrive
				...
				customerList.add(currCustomer); // put all current customer being processed in the customerList

				Event arrivalEvent = new Arrival(currCustomer.getArrivalTime(), currCustomer.getTransactionDuration(),
						currCustomer.isWalkIn(), i); // create an arrival event
				// System.out.println(arrivalEvent.getEventType());
				eventQueue.offer(arrivalEvent); // add it to the event priority queue
		...
		while (!eventQueue.isEmpty()) { // NEED TO MAKE SURE THIS IS ALWAYS THE CASE
			...
			if (currEvent.getEventType() == 0) { // CURREVENT IS A DEPARTURE
				...
				if (driveUpAnswer == 1) {
					if ((driveUp.getSize() > 0 && currTeller.getIsAtDriveUp())
						...
						depart = new Departure(clock + nxtArrivalInLine.getTransactionTime(), currTeller,
								nxtArrivalInLine.getCustomerPosition()); // create new departure event and assign to
																			// teller
					...
					// other teller already there)
						...
						if (nxtArrivalInLine == null) { // if there is no one in line, set teller to idle
							...
							eventQueue.offer(depart);
							System.out
									.println(customerList.get(nxtArrivalInLine.getCustomerPosition()).getCustomerName()
											+ " is starting their transaction.");
						}
					...
					nxtArrivalInLine = (Arrival) currTeller.getNextCustomer();
					if (nxtArrivalInLine == null) {
						currTeller.setToIdle();
					} else {
						...
						depart = new Departure(clock + nxtArrivalInLine.getTransactionTime(), currTeller,
								nxtArrivalInLine.getCustomerPosition());
						eventQueue.offer(depart);
						System.out.println(customerList.get(nxtArrivalInLine.getCustomerPosition()).getCustomerName()
								+ " is starting their transaction.");
					}
				...
				if (((Arrival) currEvent).getIsWalkIn()) { // if arrival type is a walk-in
					...
					// if currTeller has no customers in line and the current teller is not already
					// with a customer
					if (currTeller.getTellerLineLength() == 0 && !currTeller.getIsBusy()) {
						...
						Departure dep = new Departure(clock + ((Arrival) currEvent).getTransactionTime(), currTeller,
								((Arrival) currEvent).getCustomerPosition());
						System.out
								.println(customerList.get(((Arrival) currEvent).getCustomerPosition()).getCustomerName()
										+ " is starting their transaction.");
		...
		System.out.println("The total elapsed time is: " + clock);
		for (int j = 0; j < tellerArr.length; j++) {
			totalNumCustomers = totalNumCustomers + tellerArr[j].getNumCustomersHelped();
			System.out.println(tellerArr[j].getName() + " helped " + tellerArr[j].getNumCustomersHelped());
		}
		System.out.println("Total number of customers: " + totalNumCustomers);
		//System.out.println("total trans = " + countTrans);
	}
	...
	private static int findShortestLine(Teller[] tellerArr) {
		...
		int idleTellerIndex;
		int shortest = 0;
		idleTellerIndex = findIdleTeller(tellerArr);
		// if there isn't an idle teller
		if (idleTellerIndex == -1) {
